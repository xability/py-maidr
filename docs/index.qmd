...
Every render gets a new version in the URL, so a flush that changed the chart is fetched fresh, and the version it serves is marked immutable, so a frame that is merely re-attached — an `insertUI`, a tab — comes from cache. The document itself is no smaller: with `use_cdn=False` it still carries the bundle, fetched again per render rather than once per page, which is [#455](https://github.com/xability/py-maidr/issues/455). Each output holds its latest document in memory for the life of its session, ~25 KB or ~2 MB accordingly.
