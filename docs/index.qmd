...
Resolving the version makes a single outbound HTTPS request, the first time a CDN URL is built in a process. It is cached for the rest of the process and falls back to the previous `@latest` URL if it fails — so a blocked or slow network costs you a one-time delay, never an error.
