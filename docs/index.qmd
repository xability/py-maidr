...
* Stacked bar plot — several bar traces under `barmode="stack"` or `"relative"`. `relative` is Plotly's default, and what `px.bar(color=...)` leaves behind, so a stack is the ordinary way one arrives. Setting `barnorm="percent"` or `"fraction"` makes it the normalized variant, where each segment is announced as its share of the position rather than as a count.
...
* Filled area and stacked area plots — a scatter trace carrying `stackgroup`, which is what `px.area` produces. `stackgroup` is the whole signal: `fill="tozeroy"` on its own is read as a line, not an area. `groupnorm="percent"` or `"fraction"` makes it the normalized variant.
...
* Two-dimensional histogram (`go.Histogram2d`) and its contour form (`go.Histogram2dContour`), read as a lattice of binned cells.

* Contour plot (`go.Contour`), read one level curve at a time.
...
* Funnel chart (`go.Funnel`, `px.funnel`), its area form (`go.Funnelarea`), and waterfall chart (`go.Waterfall`).

* Gauge (`go.Indicator` drawing a dial) announced as a value against its range.

* Radar chart (`go.Scatterpolar`) and polar area chart (`go.Barpolar`).

* Hierarchy charts — treemap, sunburst and icicle (`go.Treemap`, `go.Sunburst`, `go.Icicle`) — navigated a level at a time.

* Sankey / alluvial diagram (`go.Sankey`), read link by link.

* Parallel coordinates (`go.Parcoords`) and parallel categories (`go.Parcats`).

* Scatter plot matrix (`go.Splom`), navigated one panel at a time.

* Choropleth map (`go.Choropleth`, `px.choropleth`) and geographic scatter and density maps (`go.Scattergeo`, `go.Scattermap`, `go.Densitymap`).

* Multi-panel plots (`make_subplots`) and facet plots, with independent axes per panel.

::: {.callout-note}
Most Plotly chart types above are **experimental**: they were added by the plot
coverage roadmap and have not yet been validated with a blind reader, so their
announcements may change without a deprecation period. See
[Stability](stability.qmd) for which plot types carry which promise, and the
[Plotly gallery](examples-plotly.qmd#sec-plotly-examples) for runnable examples.
:::
