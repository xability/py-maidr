...
If you want to temporarily use the default matplotlib renderer (e.g. for unsupported plot types or debugging), you can disable the maidr backend:

**In Jupyter notebooks:**

```python
maidr.disable()   # plt.show() now renders static inline images
maidr.enable()    # plt.show() renders accessible maidr output again
```

**In Python scripts (non-notebook):**

```python
maidr.disable()   # plt.show() now opens the native image viewer (e.g. macOS, TkAgg)
maidr.enable()    # plt.show() renders accessible maidr output again
```

::: {.callout-note}
`maidr.show()` always renders accessible output regardless of the active backend. You only need `maidr.disable()`/`maidr.enable()` to control the behavior of `plt.show()`.
:::
