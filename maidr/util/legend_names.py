"""Name a chart's groups from the legend that names their colors."""
...
def legend_of(ax: Axes | None):
    ...
    which of them names this axes' colors, and a wrong name is worse than
    ...
    **The axes' own legend always wins**, and that is the mitigation rather
    ...
    hues draw the same default color cycle, so a legend built for one of
    ...
    cannot say which names this axes' colors. And **only** where the figure
...
def names_for(ax: Axes, colors: list) -> list:
    """
    Match a list of drawn colors against the legend that names them.
    ...
    compares the three color channels alone.

    That pass is guarded: it runs only where the drawn colors are already
    ...
    swatches = [
        (_handle_color(handle), text.get_text())
        for handle, text in zip(legend.legend_handles, legend.get_texts())
    ]

    keys = [lambda color: color]
    hues = [color[:3] for color in colors if color is not None]
    if len(set(hues)) == len(hues):
        keys.append(lambda color: color[:3])

    for key in keys:
        named = _match_swatches(colors, swatches, key)
        if named:
            return [
                None if color is None else named.get(key(color)) for color in colors
            ]
    return [None] * len(colors)


def name_for(ax: Axes, color) -> str | None:
    """
    The legend's name for one drawn color.
    ...
    a layer holds one color and that color is exactly what says which level
    ...
    guard that pass needs there -- that the drawn colors are distinct without
    their alpha -- is unnecessary with one color, and
    :func:`_match_swatches` still declines a color two swatches claim.
    ...
    -------
    str or None
        The name, or ``None`` when no legend names that color.
    """
    if color is None:
        return None
    ...
    for key in (lambda one: one, lambda one: one[:3]):
        named = _match_swatches([color], swatches, key)
        if named:
            return named.get(key(color))
    return None


def _match_swatches(colors: list, swatches: list, key) -> dict | None:
    """
    Map each swatch that a drawn artist shares a color with to its name.
    ...
    colors : list
        The drawn colors.
    swatches : list
        ``(color, name)`` per legend entry, color ``None`` where the handle
        names no single one.
    key : callable
        What counts as "the same color" for this pass.

    Returns
    -------
    dict or None
        Key to name, or ``None`` when one color is claimed by two names --
        a ``style=`` legend does that, and a swatch meaning two things cannot
        name the group an artist belongs to.
    """
    drawn = {key(color) for color in colors if color is not None}
    named: dict = {}
...
def names_for_panel(ax: Axes, colors: list, faceted: bool) -> list:
    """
    ...
    panel too, and reading that as license to name would leave
    ...
    unchanged: no legend, a color no swatch claims, a color two swatches
    claim.
    ...
    list
        One name per entry, or ``None``.
    """
    if faceted and len(colors) == 1:
        return [name_for(ax, colors[0])]
    return names_for(ax, colors)
