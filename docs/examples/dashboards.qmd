...
rendered in an iframe whose document has no way to load the bundle the app
serves -- the dependency that would place it is dropped when the document is
serialized -- so the bundled `maidr.js` travels inline in the document:
correct offline, but roughly 2 MB per chart. The default (`"auto"`) loads
from the CDN and stays around 50 KB. Either way the document is fetched from
a session URL rather than pushed over the websocket, so the weight is paid
out of band; see the Shiny note under [Offline Use and the JavaScript Bundle](../index.qmd#offline-use-and-the-javascript-bundle).
...
- `tab_index=None` leaves the frame's tab order to the browser. The chart is
  ...
  makes the frame a stop as well, which is one extra Tab before the chart.

The frame is named after the chart it holds -- `"Tips on Fri, accessible
chart"` here, or `"Accessible chart"` when the chart has no title -- the same
name maidr gives its own frames in a notebook, so a screen-reader user moving
through a dashboard can tell one chart from the next. That needs a Streamlit
whose `st.iframe` takes `alt`
([streamlit#17040](https://github.com/streamlit/streamlit/pull/17040)); older
releases name every frame `st.iframe`, and `render_maidr()` leaves them to it.
...
st.iframe(
    chart_html(fig, key=day),
    height="content",
    width="stretch",
    alt=f"Tips on {day}, accessible chart",  # the frame's accessible name
)
```

Calling `st.iframe` yourself means naming the frame yourself: `alt` is what a
screen reader announces for it, and without one every chart is "st.iframe".
Drop the argument on a Streamlit that predates it.
