...
These three charts all encode a third variable as color, which is exactly
...
## Hexbin Plot [experimental] {#hexbin-plot}
...
at different x. Each bin is therefore announced by its **center** rather than
...
# `z_label` names the color axis for maidr, so a bin reads as
...
color axis to match rather than calling everything a count: `C=` replaces the
count with a reduction of the values you supply, and a numeric `bins=`
discretizes the counts so the color shows *which interval* a bin landed in.
Pass `z_label=` to name it yourself.

## Contour Plot [experimental] {#contour-plot}
