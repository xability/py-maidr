---
name: patch-expert
description: "Specialist in py-maidr's wrapt monkey-patching system and plot data extraction pipeline. Use when adding new plot types, modifying patch behavior, debugging data extraction, or understanding how matplotlib/seaborn internals are intercepted."
...
- `maidr/core/figure_manager.py` — Thread-safe singleton, maps Figure → Maidr
- `maidr/core/plot/maidr_plot_factory.py` — Factory dispatching PlotType → MaidrPlot subclass
...
- Uses mixins to extract containers, lines, collections, etc.
...
5. If it's a matplotlib/seaborn API question, use WebSearch to check the current docs
