...
twitter-card:
  title: "py-maidr: Accessible Data Visualization for Python"
description: "Make matplotlib, seaborn, Plotly, Bokeh, and Altair plots accessible to blind and low-vision users via sonification, braille, text, and AI chat. Python binding of MAIDR."
jupyter: python3
# aliases:
  # - ../index.html
...
3. Integration: maidr seamlessly integrates with the popular and mainstream data science environments (e.g., Python, pandas, and NumPy) as well as data visualization libraries in Python such as **matplotlib** and **seaborn**.
...
6. Reactivity: maidr supports widely adopted reactive and interactive computing including Jupyter Notebook, JupyterLab, Google Colab, Streamlit dashboard, and Shiny dashboard. maidr also supports interactive computing inside code editors, such as Visual Studio Code.
...
We currently support the following data visualization libraries in Python, and we are working on adding support for more libraries in the future. A plot
type marked **[experimental]** below is a prototype that may change without a
deprecation period; a type with no mark is stable (see
...
* Hexbin plot [experimental] (`ax.hexbin()`) having one x (numerical) and one y (numerical) axis and a z axis carrying what the fill encodes — each hexagon's count by default, the reduction of `C` when one is given, or the count's interval when `bins` is a number. Read as a lattice of counted cells, one row at a time; because hexagons tessellate by offsetting alternate rows, each bin is announced by its center rather than by a column index.

* Filled area plot [experimental] (`ax.stackplot()`, `ax.fill_between()`, `ax.fill_betweenx()`) having one x (numerical) and one y (numerical) axis. A band drawn from the baseline to a curve reads as an area; several stacked bands read as a stacked area [experimental], with each band's own value announced alongside the running total. A band drawn *between two curves* is not read as an area, since its content is the gap rather than either edge.
...
py-maidr ships a [Plotly](https://plotly.com/python/) adapter that builds a
MAIDR schema from a figure's traces. Use `maidr.show(fig)` (or
`maidr.save_html(fig, ...)`) to render an accessible version of any supported
Plotly figure; `plotly.express` and `plotly.graph_objects` both go through the
same entry point, since both produce the same figure object.

* One-factor bar plot (`go.Bar`, `px.bar`) having one x (categorical) and one y (numerical/count) axis.

* Dodged (grouped) bar plot — several bar traces under `barmode="group"`.

* Stacked bar plot — several bar traces under `barmode="stack"` or `"relative"`. `relative` is Plotly's default, and what `px.bar(color=...)` leaves behind, so a stack is the ordinary way one arrives. Setting `barnorm="percent"` or `"fraction"` makes it the normalized variant [experimental], where each segment is announced as its share of the position rather than as a count.

* Count plot: a bar plot over category frequencies, or `px.histogram` over a categorical column.

* One distribution histogram (`go.Histogram`, `px.histogram`) having one x (numerical) and one y (numerical/frequency) axis.

* A density curve over a histogram reads as two layers: the histogram, and the curve as an ordinary line. Plotly has no KDE trace type, so unlike `sns.kdeplot()` there is nothing here for the adapter to recognize as a density estimate — compute the curve yourself, as the gallery's example does.

* Single line plot and multi-line plot (`go.Scatter` with `mode="lines"`, `px.line`) having one x (numerical) and one y (numerical) axis.

* Step plot, recognized from Plotly's `line_shape` (`"hv"`, `"vh"`, `"hvh"`, `"vhv"`), with named y tick labels announced as level names.

* Filled area [experimental] and stacked area [experimental] plots — a scatter trace carrying `stackgroup`, which is what `px.area` produces. `stackgroup` is the whole signal: `fill="tozeroy"` on its own is read as a line, not an area. `groupnorm="percent"` or `"fraction"` makes it the normalized variant [experimental].

* Scatter plot (`go.Scatter` with `mode="markers"`, `px.scatter`) having one x (numerical) and one y (numerical) axis.

* Regression plot — a scatter trace overlaid with a fitted trend line, including `px.scatter(..., trendline=...)`.

* Vertical and horizontal box plot (`go.Box`, `px.box`) having one categorical and one numerical axis, single-class or multi-class.

* Violin plot (`go.Violin`, `px.violin`), read as a density curve plus the embedded box plot statistics.

* Heatmap (`go.Heatmap`) having one x (categorical) and one y (categorical) and z (numerical) axis.

* Two-dimensional histogram (`go.Histogram2d`) read as a lattice of binned cells, and its contour form [experimental] (`go.Histogram2dContour`), read one level curve at a time like a contour plot.

* Contour plot [experimental] (`go.Contour`), read one level curve at a time.

* Pie chart (`go.Pie`, `px.pie`), read wedge by wedge.

* Candlestick and OHLC charts (`go.Candlestick`, `go.Ohlc`) having one x (time/date) and four y (numerical) axes.

* Funnel chart [experimental] (`go.Funnel`, `px.funnel`), its area form (`go.Funnelarea`), and waterfall chart [experimental] (`go.Waterfall`).

* Gauge [experimental] (`go.Indicator` drawing a dial) announced as a value against its range.

* Radar chart [experimental] (`go.Scatterpolar`) and polar area chart [experimental] (`go.Barpolar`).

* Hierarchy charts — treemap [experimental], sunburst [experimental] and icicle [experimental] (`go.Treemap`, `go.Sunburst`, `go.Icicle`) — navigated a level at a time.

* Sankey / alluvial diagram [experimental] (`go.Sankey`), read link by link.

* Parallel coordinates [experimental] (`go.Parcoords`) and parallel categories [experimental] (`go.Parcats`).

* Scatter plot matrix (`go.Splom`), navigated one panel at a time.

* Choropleth map [experimental] (`go.Choropleth`, `px.choropleth`) and geographic scatter and density maps (`go.Scattergeo`, `go.Scattermap`, `go.Densitymap`).

* Multi-panel plots (`make_subplots`) and facet plots, with independent axes per panel.

::: {.callout-note}
A chart type marked **[experimental]** was added by the plot coverage roadmap
and has not yet been validated with a blind reader, so its announcements may
change without a deprecation period; a type with no mark is stable. See
[Plot Type Stability](stability.qmd) for which plot types carry which promise,
and the [Plotly gallery](examples-plotly.qmd#sec-plotly-examples) for runnable
examples.
:::

### Bokeh [experimental]

py-maidr ships a [Bokeh](https://bokeh.org/) adapter that, like the Plotly
one, builds a MAIDR schema in Python from the figure's own renderers and data
sources. Use `maidr.show(p)` (or `maidr.render(p)`, `maidr.save_html(p, ...)`)
with a `figure`, a `gridplot`, or a `row`/`column` of figures; install it with
`pip install "maidr[bokeh]"`. Bokeh support as a whole is **experimental**,
whichever plot type is drawn: see [Plot Type Stability](stability.qmd#bokeh-support).

* One-factor bar plot (`vbar`, `hbar`), read in the order its `FactorRange` draws the bars.

* Stacked bar plot (`vbar_stack`, `hbar_stack`), each segment announced as its own value.

* Dodged (grouped) bar plot — `vbar` renderers placed with `dodge()`, or one `vbar` on a nested `FactorRange`.

* Histogram (`quad`), vertical or sideways.

* Single line plot and multi-line plot (`line`, `multi_line`): every line on a figure is one series of one layer, named by its legend label.

* Step plot (`step`), with `mode` announced as the step direction.

* Scatter plot (`scatter`, `circle`).

* Heatmap (`rect` coloured through a colour mapper) having one x (categorical) and one y (categorical) and z (numerical) axis, named by its colour bar's title.

* Area [experimental] and stacked area [experimental] plots (`varea`, `varea_stack`).

* Multi-panel layouts (`gridplot`, `row`, `column`, `GridBox`); a `Tabs` layout is read one panel at a time, the active one.

Bokeh draws to a canvas, so the mark being read is highlighted inside the Bokeh
plot itself: a bar, bin, cell or point is selected, and a line, step or area
shows a ring on the current point. Other glyphs are left out with a warning
naming them. BokehJS is always loaded from Bokeh's CDN, as plotly.js is from
Plotly's; `use_cdn` decides where `maidr.js` comes from. See the
[Bokeh gallery](examples-bokeh.qmd#sec-bokeh-examples) for runnable examples.
...
[Python 3.9 or later](https://www.python.org/downloads/) is required to use **maidr**; the package is tested on 3.9 through 3.13. On an older interpreter `pip` reports that no matching distribution was found, rather than installing a version that cannot work. After installing Python, you can install **maidr** using the following commands in your terminal:
...
The SDK is written to `lib/dotpad-sdk-<version>/` beside the file (so
`lib/dotpad-sdk-3.0.3/` today), next to the `lib/maidr-<version>/` folder that
already travels with it, and the document declares where to find it ahead of
`maidr.js`. Ship the HTML with its `lib/` folder, as before. Every downloaded
file is verified against the size and SHA-256 recorded for the pinned commit,
and the copy includes the LGPL license text and wrapper sources the vendor asks
redistributors to keep beside the braille engine. `MAIDR_DOTPAD_SDK_DIR` moves
the cache somewhere else, for instance a directory shared by a whole machine.

The pins themselves — the SDK version, the repository and commit it is served
from, and the digest of every file — come from `maidr/static/dotpad-sdk.json`,
the manifest `maidr.js` ships and reads its own pins from. It is refreshed
together with the bundled `maidr.js`, so `maidr.DOTPAD_SDK_VERSION` always
names the release the bundle would load from the CDN.
...
One thing this does not do. Renders of the **same figure** are serialized, because `savefig` writes `fig.dpi` for its duration and two concurrent writes leave one chart drawn at the other's scale; distinct figures, which is the usual per-session shape, run in parallel. That lock is held by the render path itself rather than by this integration, so a figure is rendered one at a time however it is reached — through Shiny, through Streamlit, or from a thread of your own.

**The chart does not travel over the websocket.** A Shiny output payload used to carry the whole chart document on every reactive flush — roughly 25 KB on the default `use_cdn`, or ~2 MB with `use_cdn=False`, where the bundle is inlined into it — on the one channel every input and output shares, and a reader moving a slider paid it again per tick. `render_maidr` now keeps the document server-side and registers a session-scoped URL for it with `session.dynamic_route`; the payload is the frame pointing at that URL, and the browser fetches the chart over HTTP, off the websocket. Measured on a 50-bar chart, per flush:

| | over the websocket, before | over the websocket, now | fetched out of band |
|---|---|---|---|
| `use_cdn=True` / `"auto"` | 28 KB | **10 KB** | 51 KB |
| `use_cdn=False` | 2.2 MB | **10 KB** | 2.1 MB |

Read the table across, not only down: on the CDN settings the bytes per flush go *up* — a smaller payload plus a separate fetch of the document — and only with the bundle inlined do they fall. What falls in every case is what the websocket carries, which is the channel every input and output on the page waits on.

Every render gets a new version in the URL and the document is served `no-store`, so nothing between the reader and the app hands back an earlier chart, and a chart that may carry one reader's data is never written to the browser's disk cache. The URL is guarded by the session id in its path, as Shiny's own downloads are, and by nothing more. The document itself is no smaller: with `use_cdn=False` it still carries the bundle, fetched again per render rather than once per page, which is [#455](https://github.com/xability/py-maidr/issues/455). Each output holds its latest document in memory per session, tens of KB or ~2 MB accordingly, until the session ends.
...
| Open settings, including AI providers and API keys | Ctrl + Comma (,) | CMD + Comma (,) |
| Open the AI chat | Shift + Slash (?) | Shift + Slash (?) |
| Open the keyboard help | Ctrl + Slash (/) | CMD + Slash (/) |
| Close the settings, chat or help dialog | ESC | ESC |

Table: Keyboard Shortcuts {#tbl-shortcuts}

Three surfaces are easy to confuse, so to be explicit about what each key
opens:

* **Keyboard help** (`Ctrl + /`, `CMD + /` on Mac) lists these shortcuts. It is
  the only help dialog; there is no separate "Help menu", and no `H` shortcut.
* **Settings** (`Ctrl + ,`, `CMD + ,` on Mac) is where preferences live,
  including the **AI** tab that holds the AI providers, their API keys and the
  model each one uses.
* **AI chat** (`?`, that is `Shift + /`) opens the conversation about the
  current plot.

All three are closed with **Esc**, not by pressing the key that opened them
again: while a dialog is up, the chart's shortcuts are deliberately out of
scope so they cannot fire underneath it, and the opening chord is not rebound
inside that scope.

@tbl-shortcuts is the canonical list. Where another section of this page
mentions a key, it refers back to this table rather than restating the binding.
...
- Regular level: the braille appears similar to a bar plot, with braille values corresponding to the magnitude of the level's value for this point.
...
## Using the AI Chat {#sec-ai-chat}

Press `?` from the plot to open a conversation about the chart you are reading
(see @tbl-shortcuts). The chat answers from the same data maidr sonifies and
brailles, so you can ask about trends, outliers or a specific point without
leaving the keyboard. **Esc** closes it and returns focus to the plot.

The chat runs against a provider you supply, so it needs to be configured once
before the first question:

1. Activate the plot by pressing **Tab** or clicking on the plot area.
2. Open **Settings** (`Ctrl + ,`, `CMD + ,` on Mac) and go to the **AI** tab.
3. Enable one or more providers and enter each one's API key. maidr supports
   **OpenAI**, **Anthropic Claude** and **Google Gemini**, and it can also talk
   to a local [Ollama](https://ollama.com/) server, which takes a server URL in
   place of a key and keeps the conversation on your own machine.
4. Choose a model for each enabled provider. The dropdown is filled from that
   provider's own models API using the key you entered, so it offers whatever
   is available to your account today — including models released after this
   page was written. maidr ships a small curated list as a fallback for when a
   provider cannot be reached; it is not a limit on what you can pick.
5. Close the dialog and press `?` to start chatting.

Enabling more than one provider is supported: a question goes to all of them at
once, and you can compare the answers.

::: {.callout-note}
## Your API keys are stored in your browser

maidr saves its settings, API keys included, in the browser's `localStorage`
under the origin serving the page, and calls the provider directly from the
browser with the key you entered. Nothing you type into the settings dialog
reaches your Python process, and no key is written into the file
`maidr.save_html()` produces.

Because `localStorage` persists, a key stays in that browser between visits.
On a shared or public machine, use **Reset** in the settings dialog when you
are done.
:::
...
- [py-maidr for Python](https://py.maidr.ai/): this package, for matplotlib, seaborn, Plotly, Bokeh, and Altair (PyPI package `maidr`, source on [GitHub](https://github.com/xability/py-maidr)).
