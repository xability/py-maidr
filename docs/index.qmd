...
description: "Make matplotlib, seaborn, plotly, and altair charts accessible to blind and low-vision users with sonification, braille, text, and AI chat modalities."
jupyter: python3
# aliases:
  # - ../index.html
...
Resolving the version makes a single outbound HTTPS request, the first time a CDN URL is built in a process. It is bounded by `MAIDR_CDN_TIMEOUT` (3 seconds total), cached for the rest of the process, and falls back to the previous `@latest` URL if it fails — so a blocked or slow network costs you a one-time delay, never an error.

In sandboxed or egress-blocked environments (CI, air-gapped machines), set `MAIDR_CDN_VERSION=latest` to skip the lookup entirely, or use `use_cdn=False`, which never makes any network request.
:::
...
The warning needs to know what the current published release *is*, and it will never make a network request to find out — that would defeat `use_cdn=False`. So it only fires when the session already knows: because something used the CDN, or because you pinned a version.

A session that only ever uses `use_cdn=False`, with no pin, stays silent no matter how old its bundle is. That is the air-gapped case, and it is the one this warning cannot reach by itself. Check it explicitly instead:
:::
...
| `MAIDR_CDN_VERSION` | Pin the CDN version: a version such as `3.74.0`, `bundled`, or `latest` to skip resolution. If `bundled` is set but the installed `VERSION` file is missing or unreadable, py-maidr warns and falls back to `latest` without making a network request. |
| `MAIDR_CDN_TIMEOUT` | Seconds allowed for the version lookup, shared across attempts (default `3`). Values of `0` or less fall back to the default; no upper bound is imposed, so a large value can make a first render block for that long. |
