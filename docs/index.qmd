...
**The render itself still blocks, though.** The heading above is about the version *lookup*; do not read it as "nothing blocks the loop". `maidr.render()` is synchronous and CPU-bound — matplotlib's SVG writer, then lxml — so it runs on the event loop and every other session on that worker waits. Measured on a trivial two-bar chart: about 57 ms per render, during which an `asyncio.sleep(0.001)` ticker managed 86 wakeups against 594 in an idle window of the same length. Starved, not frozen — but a large scatter or heatmap is worse, and the cost is paid per session per input change. Tracked in [#454](https://github.com/xability/py-maidr/issues/454).
:::
