...
In sandboxed or egress-blocked environments (CI, air-gapped machines), set `MAIDR_CDN_VERSION=latest` or `MAIDR_USE_CDN=false` in the environment **before importing maidr**.

Reach for the environment variables rather than `use_cdn=False` on a render call. The per-call argument is honoured — that render makes no request — but in a notebook `import maidr` calls `init_notebook()` under the default `auto` mode, which resolves before you have called anything. Only configuration that is in place at import time prevents that.
...
`MAIDR_BUNDLE_STALE_WARNING=0` to turn it off, or add a narrower
`filterwarnings` entry for it.
:::
