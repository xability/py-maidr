...
The bundled `maidr.js` is refreshed when py-maidr is released, so it can fall behind between releases. Since the offline fallback is what runs whenever the CDN is disabled or unreachable, py-maidr warns once per session if the bundled copy has drifted a long way behind the published release. Upgrading py-maidr picks up a fresher bundle.

Check the gap yourself at any time:
...
| `MAIDR_CDN_TIMEOUT` | Total seconds allowed for the version lookup (default `3`). |
